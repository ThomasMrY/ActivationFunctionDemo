...
def train_test(training,file_name):
	...
	class Net(nn.Module):
		...
		def forward(self,x):
			x = F.max_pool2d(F.tanh(self.cov1(x)),(2,2))
			x = F.max_pool2d(F.tanh(self.cov2(x)),(2,2))
			x = x.view(-1,16*4*4)
			x = F.tanh(self.fc1(x))
			x = F.tanh(self.fc2(x))
			x = self.fc3(x)
		...
		def forward_apx(self,x):
			x = F.max_pool2d(ops.tanh_apx(self.cov1(x),file_name), (2, 2))
			x = F.max_pool2d(ops.tanh_apx(self.cov2(x),file_name), (2, 2))
			x = x.view(-1, 16 * 4 * 4)
			x = ops.tanh_apx(self.fc1(x),file_name)
			x = ops.tanh_apx(self.fc2(x),file_name)
			x = self.fc3(x)
			return x
